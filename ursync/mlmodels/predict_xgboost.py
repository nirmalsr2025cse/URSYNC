# mlmodels/predict_xgboost.py
"""
High-Precision XGBoost Prediction Script for Tender Conflict Detection
-----------------------------------------------------------------------
Loads trained .pkl XGBoost bundle and performs high-precision inference:
...
class XGBoostConflictPredictor:
    def __init__(self, model_pkl_path=MODEL_PKL_PATH, preprocessor_pkl_path=PREPROCESSOR_PKL_PATH):
        if not os.path.exists(model_pkl_path):
            raise FileNotFoundError(f"Trained XGBoost .pkl model not found at {model_pkl_path}. Run train_xgboost.py first.")
...
if __name__ == '__main__':
    if not os.path.exists(MODEL_PKL_PATH):
        print(f"Model .pkl not found at {MODEL_PKL_PATH}. Please run train_xgboost.py first.")
    else:
        predictor = XGBoostConflictPredictor()
        ...
        print("=" * 65)
        print("High-Precision XGBoost Conflict Prediction Demo (.pkl)")
        print("=" * 65)
