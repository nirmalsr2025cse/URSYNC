...
// Usage: navigate('/tender-view', { state: { tender } })
// Same page as TenderDetailsView.jsx — works with DB-backed tender objects
// (the shape controllers' toCardShape() returns): id/tenderCode, title,
// description, image, documentUrl, department, departmentCode, organization,
// category, location, taluk, village, latitude, longitude, value,
// estimatedValue, startDate, closingDate, duration, status, isCancelled,
// cancelledReason, cancelledAt, isRetendered, retenderedAt.
...
const STATUS_CONFIG = {
  ...
  'Upcoming':  { bg: 'bg-amber-50',   text: 'text-amber-700',   border: 'border-amber-200',   dot: 'bg-amber-500'   },
  ...
  return (
    <div className="p-4 lg:p-6 space-y-5 min-h-screen animate-fade-in">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <StatCard icon={<CurrencyStatIcon />} label="Tender Value" value={tender.value} color="blue" />
        <StatCard icon={<OrgStatIcon />} label="Organisation" value={tender.organization} color="emerald" />
        <StatCard icon={<PinStatIcon />} label="Location" value={tender.location} color="amber" />
        <StatCard
          icon={<ClockStatIcon />}
      ...
      {!isCompleted && !tender.isCancelled && days !== null && days >= 0 && (
        <div className={[
          ...
          days <= 3 ? 'bg-red-50 border-red-200' : 'bg-[#FFF2DB] border-[#FFE5BF]',
        ].join(' ')}>
